...
async function handleList(queue: TaskQueue, args: string[]): Promise<void> {
  ...
  if (status) {
    ...
    console.log('\n📋 All Tasks:');
    const taskData = tasks as { pending: unknown[]; processing: unknown[]; completed: unknown[]; failed: unknown[] };
    ['pending', 'processing', 'completed', 'failed'].forEach((s) => {
      const taskList = (taskData[s as keyof typeof taskData] || []) as Array<{ id: string; description: string }>;
