...
export class TaskAPI {
  ...
  start(): void {
    this.server = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
      ...
      try {
        ...
        } else if (path.startsWith('/api/tasks/') && path.endsWith('/retry') && method === 'POST') {
  ...
  private async handleAddTask(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    req.on('end', async () => {
      try {
        const data = JSON.parse(body) as { description?: string; [key: string]: unknown };
   ...
   */
  private async handleListTasks(req: IncomingMessage, res: ServerResponse, url: URL): Promise<void> {
   ...
   */
  private async handleGetTask(req: IncomingMessage, res: ServerResponse, path: string): Promise<void> {
   ...
   */
  private async handleRetryTask(req: IncomingMessage, res: ServerResponse, path: string): Promise<void> {
   ...
   * Handle errors
   */
  private handleError(req: IncomingMessage, res: ServerResponse, error: Error): void {
