...
export class AgentWorker {
  ...
  async start(): Promise<void> {
    ...
    console.log(`🚀 Agent Worker started (polling every ${this.pollInterval / 1000}s)`);
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      // Process the task
      const result = await this.taskManager.processTask(task.description, task.options);
  ...
  private async logStats(): Promise<void> {
    ...
    console.log(`   Processed: ${this.stats.processed} (${this.stats.succeeded} succeeded, ${this.stats.failed} failed)`);
    console.log(`   Queue: ${queueStats.pending} pending, ${queueStats.processing} processing`);
